...
import com.example.abe.dcpabe.acceess.AccessStructure.MatrixElement;
...
public class DCPABE {
    ...
    public static void authorityUpdate(AuthorityKeys authorityKeys, GlobalParameters GP, String... attributes) {
        Pairing pairing = PairingFactory.getPairing(GP.getPairingParameters());
        Element eg1g1 = pairing.pairing(GP.getG1(), GP.getG1()).getImmutable();
        for (String attribute : attributes) {
            Element ai = pairing.getZr().newRandomElement().getImmutable();
